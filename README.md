<img src="Files/img/Azure%20Architecture.jpg" alt="Azure Architecture" width="1000"/>
...
<img src="Files/img/Storage_Account.png" alt="Storage_Account" width="400"/>  
...
<img src="Files/img/Dictionaries.png" alt="Data Lake Dictionaries" width="900"/>  
...
<img src="Files/img/Launch_Workspace.png" alt="Launch Workspace" width="900"/>  

3. Inside the workspace, create a **Compute (Cluster)**. This step is essential to run Spark jobs.  

<img src="Files/img/Compute.png" alt="Databricks Compute" width="900"/>  
...
<img src="Files/img/Add_Files.png" alt="Add_File" width="400"/>  

3. Upload the dataset (e.g., into `Legacy Products`).  
4. Under **Settings → Advanced → DBFS File Browser**, enable **On**.  

<img src="Files/img/Settings.png" alt="Databricks Settings" width="900"/>  
...
<img src="Files/img/App_Registration.png" alt="App_Registration" width="900"/>  
...
<img src="Files/img/ClientSecret.png" alt="Client Secret" width="900"/>  
...
<img src="Files/img/AccessControl.png" alt="Access Control" width="900"/>  

3. Select the role **Storage Blob Data Contributor**.  
4. Assign it to the **App Registration Display Name**.  

<img src="Files/img/RoleAssignment.png" alt="Role Assignment" width="900"/>  
...
<img src="Files/img/File_Path.png" alt="File Path" width="900"/>  

---

## 11. Automate Notebooks with Databricks Jobs  

<img src="Files/img/ETL.png" alt="File Path" width="900"/>  

Once the environment and notebooks are ready, you can **automate the ETL process** with Databricks Jobs.  

1. Sign in to **Databricks Workspace**.  
2. In the left sidebar, under **Data Engineering**, click **Job Runs**.  
3. Click **Create Job** to start setting up the ETL workflow.  

### Step 1: Configure the first task (Bronze)  
- Give the job a name, e.g., **ETL Pipeline Job**.  
- Add a task name, e.g., **Data Ingestion**.  
- Task Type → **Notebook**.  
- Source → **Workspace**.  
- Select the notebook path for the **Bronze layer**.  

### Step 2: Configure the job cluster  
- Name: **Data Ingestion Cluster**.  
- Cluster mode: **Single Node**.  
- Access: **Single User**.  
- Runtime: keep the default runtime.  
- Node type: **Standard D3V2 (14GB memory, 4 cores)**.  
- Click **Confirm**.  

### Step 3: Add Silver and Gold tasks  
- Click **Add Task**, select **Notebook**, and repeat the process for the **Silver notebook**.  
- Set the dependency: **Run if dependencies → All succeeded**.  
- Add another task for the **Gold notebook**, depending on the Silver task.  

### Step 4: Configure schedules & alerts  
- Add schedules (e.g., daily or hourly triggers).  
- Add notifications (e.g., email alerts on failure).  

### Step 5: Run and monitor the pipeline  
- Click **Run Now** to execute the job manually the first time.  
- Navigate to the **Run section** to monitor progress.  
- Each task will display its status (Pending, Running, Completed, or Failed).  

This setup ensures a **fully automated ETL pipeline** across Bronze → Silver → Gold layers, improving reproducibility and reliability.  
